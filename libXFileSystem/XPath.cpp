...
//-----------------------------------------------------------------------------
// Renvoie le nom du repertoire parent (vide si pas de répertoire)
//-----------------------------------------------------------------------------
...
{//*****non testé !!!
	...
	int pos = (int) P.rfind('.'); 
	if(pos != std::string::npos)//on enlève le nom de fichier
...
//-----------------------------------------------------------------------------
//renvoie le repertoirede niveau  immédiatement supérieur
//-----------------------------------------------------------------------------
...
std::string XPath::NameNoExt(const char* filename)
{
	...
	return P.substr(0,P.rfind('.'));//enlève les doubles extensions (.ori.xml par exemple)
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Renvoie la première extension d'un fichier
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Renvoie l'extension complète d'un fichier (ori.xml)
...
//-----------------------------------------------------------------------------
// change l'extension ou ajoute  une extension à un fichier
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// insère une chaine dans le nom du fichier avant l'extension 
...
{
	std::string R = root;
	SubPathSep(R);//root ne doit pas avoir le \ à la fin
...
//-----------------------------------------------------------------------------
// Renvoie un chemin relatif dans le cas où path est un repertoire
//à utiliser pour être sur d'avoir le \ à la fin -> ex : ".\orthos_imagettes\"
//-----------------------------------------------------------------------------
std::string XPath::FolderRelative(const char* root, const char* path)
{
	...
	// path = C:\toto\titi
        // => .\titi
	//çà c'est ok lorsque le repertoire est un sous repertoire, on a bien le \ a la fin
	pos = (int)P.find(R);
	if (pos == 0) {
		if (P[R.length()] == PathSep) {//pour vérifier le cas e:\toto  e:\tototiti\tata
			Rel = "." + P.substr(R.length());
			return Rel;
		}
	}
	...
	//çà c'était pas bon dans la fonction "relative", on avait pas le \ a la fin
	std::string root1 = R.substr(0, R.find(PathSep));
	std::string root2 = P.substr(0, P.find(PathSep));
	if (root1.compare(root2) != 0)	// Pas le meme disque
		return P;
...
std::string XPath::Absolute(const char* root, const char* path)
{
	...
	//ajout jpp pour compatibilité avec ancienne version qui rajoutait un \ en trop
...
//à utiliser quand on est pas sûr d'avoir le \ à la fin -> ex : ".\orthos_imagettes\"
//-----------------------------------------------------------------------------
